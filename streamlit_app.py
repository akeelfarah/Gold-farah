...
# --- 1. حل مشكلة إثبات الملكية (ads.txt) لـ Google AdSense ---
if "ads" in st.query_params:
    # ملاحظة: تأكد أن اسم الملف ads.txt بحروف صغيرة في GitHub
...
# --- 3. حقن كود الإعلانات (AdSense Script) ---
adsense_code = """
...
# --- 4. التنسيق الجمالي (CSS) وإخفاء علامات Streamlit ---
st.markdown("""
    <style>
    /* إخفاء القوائم والشريط السفلي الأحمر */
    #MainMenu, footer, header {visibility: hidden;}
    .stDeployButton {display:none;}
    
    /* تنسيق الخلفية والألوان الذهبية */
...
with col_main:
    # خانة إدخال السعر (القيمة الافتراضية 4497.0)
    ounce_input = st.number_input("أدخل سعر الأونصة العالمي الحالي ($)", value=float(live_price))
    
    # حساب العيارات بناءً على وزن الأونصة (31.1035 جرام)
    troy_ounce = 31.1035
...
with col_side:
    # الأزرار الجانبية ووسائل التواصل
    st.markdown(f"""
...
# --- 7. قسم سياسة الخصوصية (أساسي لقبول أدسنس) ---
st.markdown("<br>", unsafe_allow_html=True)
with st.expander("📄 سياسة الخصوصية واستخدام البيانات"):
    st.write("""
    هذا الموقع يوفر أسعار الذهب للأغراض المعلوماتية فقط. نحن لا نجمع أي بيانات شخصية. 
    يتم استخدام كوكيز الطرف الثالث عبر Google AdSense لتحسين تجربة الإعلانات.
    """)
